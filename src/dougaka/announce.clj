...
(defn -main [& [path episode-id title]]
  (when-not (and path (.exists (io/file path)))
    (binding [*out* *err*] (println "usage: clojure -M:dev -m dougaka.announce <video.mp4> [episode-id] [title]"))
    (System/exit 1))
