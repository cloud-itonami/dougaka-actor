(ns dougaka.aozora
  "Real app-aozora Publisher for dougaka — creates a record in the
  com.etzhayyim.apps.dougaka.video collection on an aozora PDS via the
  AT Protocol com.atproto.repo.createRecord XRPC, authenticated by a depth-1
  self-minted CACAO (the actor's own did:key). Ported from minidrama.aozora
  ...
  When produced media exists, the video record carries the
...
(defn aozora-publisher
  "Returns a `dougaka.publisher/Publisher` that creates video records on
  the aozora PDS. opts:
