(ns dougaka.cacao
  "Agent-side CACAO issuance (JVM). The dougaka actor mints its OWN
  ...
  `minidrama.cacao` (itself from `tashikame.cacao` / `tsumugu.cacao`, keep in
  sync); the SIWE/wire builders below are a faithful copy of the proven
  byte-exact pure functions in `kotoba.cacao` (kotoba-auth / kotoba-wasm),
  and the crypto is JDK Ed25519 + a minimal CBOR encoder.

  Per-actor key model: dougaka generates + persists its OWN Ed25519 key
...
;; to `kotobase.cid/canonical-graph`. Ported from minidrama.cacao.
...
(defn canonical-graph
  "The deterministic graph CID for one of dougaka's databases. The edge
...
(def default-db-name
  "dougaka's primary database — the video ledger."
  "dougaka")
...
(defn load-or-create-identity!
  "Per-actor key: load dougaka's persisted Ed25519 identity at `path`, or
