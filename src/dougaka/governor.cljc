(ns dougaka.governor
  "DougakaGovernor — the independent censor that earns a VideoLLM proposal
  the right to be committed (and, phase permitting, announced). SEPARATE
  system from the VideoLLM (defense in depth: never trust the generator to
  have been the only gate). Mirrors minidrama.governor / tashikame.governor's
  shape (HARD → HOLD, no override; SOFT → recorded warning, still commits).
  The gate set is shared with minidrama verbatim — the same vertical-short
  format, content and budget constraints apply to general short video.

  Gates (ADR-2607071300 family / superproject ADR-2607162200 Layer D):
  HARD (never commit):
    :no-actuation         proposal :effect ≠ :production (dougaka only plans)
    ...
    :budget-exceeded      shots × cost-per-shot > video budget (context)
...
(defn- likeness? [episode]
  ...
  ;; richer likeness/brand detector. No real persons, no brands.
...
(defn check
  "Censors a VideoLLM proposal. Returns {:ok? :violations [hard]
  ...
  (let [effect  (:effect proposal)
        ...
        hard (cond-> []
               ...
               (conj {:rule :no-actuation
                      :detail "dougaka only plans production; :effect must be :production"})
               ...
               (conj {:rule :likeness
                      :detail "real-person likeness / brand use — no real persons, no brands"})
...
(defn verdict->disposition
  "Map a DougakaGovernor verdict to a base disposition. HARD → :hold, else
  :commit."
  [verdict]
